...
Factory function
*/
module.exports = function (stoxumlib, sjcl) {

var Remote = stoxumlib.Remote;
var Seed = stoxumlib.Seed;
var Account = stoxumlib.Account;
var UInt160 = stoxumlib.UInt160;

// Message class (static)
var Message = {};

Message.hashFunction = sjcl.hash.sha512.hash;
Message.MAGIC_BYTES = 'Stoxum Signed Message:\n';
 ...
 *  the string 'Stoxum Signed Message:\n' prepended.
 *
 ...
 *         secret_key
 *  @param {StoxumAddress} [The first key] account Field to specify the signing
 ...
 *  @param {sjcl.ecc.ecdsa.secretKey|Any format accepted by Seed.from_json}
 *          secret_key
 *  @param {StoxumAddress} [The first key] account Field to specify the
 ...
 *  The stoxum-lib remote is used to check that the public
 ...
 *  @param {StoxumAddress} data.account
 *  @param {Base64-encoded String} data.signature
 *  @param {stoxum-lib Remote} remote
...
Message.verifyHashSignature = function(data, remote, callback) {
  ...
  if (!account || !UInt160.from_json(account).is_valid()) {
    return callback(new Error('Account must be a valid stoxum address'));
  }
